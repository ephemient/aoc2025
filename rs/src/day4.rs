...
pub fn solve(input: &str) -> (usize, usize) {
    let mut points = input
        ...
        .flat_map(|(y, line)| {
            line.char_indices()
                .filter_map(move |(x, c)| if c == '@' { Some(((x, y), -1)) } else { None })
        })
        .collect::<HashMap<_, _>>();
    for (x, y) in points.keys().copied().collect::<Vec<_>>() {
        for x in x.saturating_sub(1)..=x.saturating_add(1) {
            for y in y.saturating_sub(1)..=y.saturating_add(1) {
                points.entry((x, y)).and_modify(|count| *count += 1);
            }
        }
    }
    let mut removals = points
        .extract_if(|_, count| *count < 4)
        .map(|(point, _)| point)
        .collect::<Vec<_>>();
